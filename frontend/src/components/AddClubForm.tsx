...
const AddClubForm = ({ onClubAdded }: Props) => {
    ...
    const fetchData = async () => {
      try {
        ...
        const available = studentsData.filter(
          (student) => !presidentIds.has(student.id)
        );
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const chosenPresidentId = Number(presidentId);

    try {
      await createClub({
        name,
        description,
        presidentId: chosenPresidentId,
      });
      ...
      setAvailableStudents((prev) => prev.filter((s) => s.id !== chosenPresidentId));
    } catch (error: any) {
      console.error(error);
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        <select
          value={presidentId === '' ? '' : String(presidentId)}
          onChange={(e) =>
            setPresidentId(e.target.value === '' ? '' : Number(e.target.value))
          }
          required
          style={{ marginBottom: '10px', display: 'block' }}
        >
        ...
        {availableStudents.length === 0 && (
          <p style={{ color: 'orange' }}>
            Aucun étudiant éligible (tous sont déjà présidents ou aucun étudiant existant).
          </p>
        )}
      </div>
